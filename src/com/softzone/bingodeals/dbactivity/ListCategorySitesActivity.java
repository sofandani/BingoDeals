...
/*
 * show the list of island wide places 
 * getting from the database
 */

public class ListCategorySitesActivity extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		type = i.getStringExtra(TAG_TYPE);
		...
		// select a single site and start siteDetailActivity
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				// Starting new intent (SiteDetailActivityActivity)
	...
	class LoadAllSites extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(ListCategorySitesActivity.this);
			...
			try {
				...
				if (success == 1) {
					...
					for (int i = 0; i < sites.length(); i++) {
						...
						String cate = c.getString(TAG_TYPE);
						if (!type.equals(cate)) {
							continue;
						}
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					ListAdapter adapter = new SimpleAdapter(
							ListCategorySitesActivity.this, sitesList,
							R.layout.list_site, new String[] { TAG_LID,
